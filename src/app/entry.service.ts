...
export class EntryService {
  ...
  constructor() {
    this.entries = this.entryChanges.scan(
        [], (oldEntries, f: ChangeFunc) => { return f(oldEntries); });

    this.selectedIds = this.selectedChanges.scan(
        [], (oldIds, f: IdChangeFunc) => { return f(oldIds); });

    this.selectedEntries =
        this.selectedIds.combine(this.entries, (ids, entries) => {
          // Filter all of the entries whose ids are not in selectedIds.
          return entries.filter(e => { return ids.indexOf(e.id) != -1; }, this);
        });
  }

  AddEntries(entries: [Entry]): void {
    this.entryChanges.push(
        (oldEntries: [Entry]) => { return oldEntries.concat(entries); });
  }

  AddSelection(id: string) {
    this.selectedChanges.push(
        (oldIds: [string]) => { return oldIds.concat(id); });
  }

  RemoveSelection(id: string) {
    this.selectedChanges.push((oldIds: [string]) => {
      return oldIds.filter((oldId: string) => { return oldId != id; });
    });
  }
}
